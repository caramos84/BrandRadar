...
PROMO_TERMS = {
    ...
    "black friday", "cyber", "hot sale", "exclusivo", "exclusivos", "regalo", "regalos", "bono", "bonos",
}
...
CTA_TERMS = {
    ...
    "entra", "paga", "pagala", "págala", "estrena", "recibe", "usa", "usando", "shop", "buy", "get", "claim", "order", "learn more",
}
URGENCY_TERMS = {"hoy", "ya", "ultimo", "ultimos", "ahora", "tiempo limitado", "solo hoy", "por tiempo limitado"}
FINANCING_TERMS = {"cuota", "cuotas", "financia", "financiacion", "financiación", "credito", "crédito", "paga", "pagala", "págala", "pago"}
...
BRAND_TERMS = {"logo", "marca", "marcas", "aliados", "aliadas", "comercios", "seleccionados", "samsung", "addi", "decathlon", "brand", "brandmark", "isotype", "wordmark"}
...
def compute_asset_signals(
    ...
    event_count = _count_terms(normalized_text, {"black friday", "black days", "cyber", "hot sale", "sale"})
    numeric_density = _clamp01((numeric_tokens / token_count) * 3.0)
    promo_presence = _clamp01(
        min(1.0, promo_count) * 0.82
        + min(1.0, financing_count / 2.0) * 0.12
        + min(1.0, event_count) * 0.18
    )
    base_commercial = _clamp01(
        min(1.0, promo_count / 2.0) * 0.26
        + min(1.0, price_count / 2.0) * 0.15
        + min(1.0, cta_count / 2.0) * 0.20
        + min(1.0, financing_count / 2.0) * 0.18
        + min(1.0, event_count) * 0.10
        + min(1.0, urgency_count / 2.0) * 0.06
        + min(1.0, condition_count / 3.0) * 0.06
        + numeric_density * 0.08
        + min(1.0, symbol_count / 3.0) * 0.05
    )
    ...
    if event_count and cta_count:
        combination_bonus += 0.08
    commercial_pressure = _clamp01(base_commercial + combination_bonus * 0.65)
    conversion_intent = _clamp01(
        commercial_pressure * 0.74
        + promo_presence * 0.10
        + min(1.0, cta_count / 2.0) * 0.06
        + min(1.0, financing_count / 2.0) * 0.05
        + min(1.0, text_block_count / 10.0) * 0.03
        + min(1.0, (brand_count + product_count) / 3.0) * 0.02
    )
    ...
    language_stress = _clamp01(
        ...
        + min(1.0, (promo_count + cta_count + urgency_count + event_count) / 4.0) * 0.08
    )
