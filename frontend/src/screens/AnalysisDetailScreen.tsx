...
export function AnalysisDetailScreen({ analysis, token, onBack }: Props) {
  ...
  const attentionHeatmapPath = useMemo(() => {
    if (!selectedAsset?.vision_data_json) return null;
    try {
      const parsed = JSON.parse(selectedAsset.vision_data_json) as { attention_heatmap_path?: string | null };
      return parsed.attention_heatmap_path ?? null;
    } catch {
      return null;
    }
  }, [selectedAsset?.vision_data_json]);
              ...
              <div className="drawer-analysis-panel">
                ...
                {activeDrawerPanel === 'heatmap' && (
                  <>
                    ...
                    {attentionHeatmapPath ? (
                      <div className="attention-heatmap-shell">
                        <img src={`${API_BASE_URL}${attentionHeatmapPath}`} alt="Attention heatmap" className="attention-heatmap-image" />
                      </div>
                    ) : (
                      <p className="asset-meta-secondary">Attention heatmap pending.</p>
                    )}
