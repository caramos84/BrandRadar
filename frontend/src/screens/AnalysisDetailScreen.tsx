                ...
                {activeDrawerPanel === 'radar' && (
                  <>
                    ...
                    <div className="radar-chart-shell">
                      <svg className="radar-chart" viewBox="0 0 320 320" role="img" aria-label="Radar chart">
                        {[0.25, 0.5, 0.75, 1].map((ring) => {
                          const points = radarScores.map((_, index) => {
                        ...
                        {radarScores.map((_, index) => {
                          ...
                          return <line key={`axis-${index}`} x1={160} y1={160} x2={x} y2={y} className="radar-axis" />;
                        })}
                        <polygon
                          points={radarScores.map((item, index) => {
                            const angle = -Math.PI / 2 + (Math.PI * 2 * index) / radarScores.length;
                            const x = 160 + Math.cos(angle) * 105 * clamp01(item.value);
                            const y = 160 + Math.sin(angle) * 105 * clamp01(item.value);
                            return `${x},${y}`;
                          }).join(' ')}
                        ...
                        {radarScores.map((item, index) => {
                          ...
                          return (
                            <g key={`pt-${item.label}`}>
                              <circle cx={x} cy={y} r={3.2} className="radar-point" />
                              <text x={lx} y={ly} textAnchor="middle" className="radar-label">{item.label}</text>
                            </g>
                          );
                        })}
                      </svg>
